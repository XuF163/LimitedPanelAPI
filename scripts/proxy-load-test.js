...
async function main() {
  ...
  const concurrency = Math.max(1, Math.min(50, toInt(process.env.ENKA_CONCURRENCY || process.env.CONCURRENCY, 20)))
  ...
  console.log(`\n===== proxy load test =====`)
  console.log(`game=${game} uidStart=${uidStart} count=${count} delayMs=${delayMs} concurrency=${concurrency}`)
