...
2. `EnkaClient`
   ...
   - 不做代理（按“直连 Enka + 失败退避 + 永久跳过坏 UID”策略）
3. `SampleCollector`
...
QA 测试流程已落地（见 `scripts/qa-flow.js`，以及 `config/README.md` 的示例命令）。
...
- 核验命令：`node src/cli.js verify:liangshi --uid 100000000 --games gs,sr`（绝区零暂不评估）
