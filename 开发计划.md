...
### 1.5 meta 来源（qsyhh CNB 仓库）

miao-plugin 的 meta 外置说明（可直接复用）：
...
- `GET /healthz`
- `GET /presets/:game/:uid.json`（game=gs|sr）
...
2. **隐私**：仅保存“构筑样本”（角色 id、圣遗物组合等），避免持久化昵称/签名/头像等玩家信息；raw 响应可配置是否落盘。
3. **数据偏差**：样本来自特定 UID 段会导致偏差；需要支持多来源 UID 列表与“样本量不足回退策略”。
4. **meta 版本漂移**：导出 JSON 必须与解析端使用的 meta 版本一致（至少同一 major）；建议在导出中记录 meta commit。
...
3. `EnkaClient`：gs/sr 拉取 + 限速 + raw 落盘
