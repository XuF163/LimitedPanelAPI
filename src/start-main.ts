...
import { setProxyPool } from "./runtime/state.js"
...
async function waitForProxyUrls({ min = 1, timeoutMs = 30_000, intervalMs = 200 } = {}) {
  const need = Math.max(0, Math.trunc(Number(min) || 0))
  if (need <= 0) return true

  const timeout = Math.max(0, Number(timeoutMs) || 0)
  const interval = Math.max(50, Math.min(2000, Number(intervalMs) || 200))
  const start = Date.now()

  while (true) {
    const cur = parseProxyUrls().length
...
async function main() {
  ...
  const games = (() => {
    const raw = String(process.env.GAMES || process.env.GAME || "").trim()
    if (!raw) return [ "gs", "sr", "zzz" ]
    const list = raw
      .split(/[,;\s]+/)
      .map((s) => String(s || "").trim().toLowerCase())
      .filter(Boolean)
      .map((g) => (g === "genshin" ? "gs" : g === "starrail" ? "sr" : g))
      .filter((g) => [ "gs", "sr", "zzz" ].includes(g))
    return list.length ? Array.from(new Set(list)) : [ "gs", "sr", "zzz" ]
  })()
  ...
  const proxyEnabled = envBool("PROXY_ENABLED", Boolean(cfg?.proxy?.enabled ?? false))
  let preservedProxyUrlsEnv = String(process.env.PROXY_URLS || "").trim()
  const bootstrapSubHttpProxyEnabled = envBool("PROXY_BOOTSTRAP_SUB_HTTP_PROXY", true)
  const bootstrapProxyUrl = String(process.env.PROXY_BOOTSTRAP_PROXY || cfg?.proxy?.subscription?.httpProxy || "").trim()
  if (proxyEnabled && !preservedProxyUrlsEnv && bootstrapSubHttpProxyEnabled && bootstrapProxyUrl) {
    // Seed a working proxy immediately (e.g. Clash at 127.0.0.1:10808) while the local pool boots.
    // Keep it as "preserved" so failures/cleanup will fall back to it instead of clearing PROXY_URLS.
    preservedProxyUrlsEnv = bootstrapProxyUrl
    process.env.PROXY_URLS = bootstrapProxyUrl
    logProxy.info(`启动代理：${bootstrapProxyUrl}`)
  }
  ...
  const cleanupProxyPool = async () => {
    try {
      ...
      await pool?.close?.()
      setProxyPool(null)
      // Restore env to prevent stale (already closed) local proxies from being used later.
      setProxyUrlsEnv([])
    } catch {}
  }
  ...
  // When proxy is required, enforce it before Enka sampling.
  if (proxyRequired) await proxyPoolPromise
  ...
  try {
    ...
    // If Enka is going to run and proxy is enabled, wait briefly for the proxy pool to publish PROXY_URLS.
    // This avoids the first few requests going direct (often timing out) while the pool is still booting.
    const proxyEnabled = envBool("PROXY_ENABLED", Boolean(cfg?.proxy?.enabled ?? false))
    const waitProxyEnabled = envBool("PROXY_WAIT_READY", Boolean(cfg?.proxy?.subscription?.waitReady ?? true))
    const waitMin = Math.max(0, Math.min(50, envNum("PROXY_WAIT_READY_MIN", Number(cfg?.proxy?.subscription?.waitReadyMin ?? 1))))
    const waitMs = Math.max(0, Math.min(5 * 60_000, envNum("PROXY_WAIT_READY_MS", Number(cfg?.proxy?.subscription?.waitReadyMs ?? 60_000))))
    if (proxyEnabled && waitProxyEnabled && !proxyRequired && enkaGames.length > 0 && waitMin > 0 && parseProxyUrls().length < waitMin) {
      logProxy.info(`等待代理就绪：min=${waitMin} timeoutMs=${waitMs}`)
      const ok = await waitForProxyUrls({ min: waitMin, timeoutMs: waitMs })
      const got = parseProxyUrls().length
      if (!ok) logProxy.warn(`等待代理就绪超时：got=${got}/${waitMin}；将继续直连/可能超时`)
      else logProxy.info(`代理就绪：${got}`)
    }
    ...
    if (enkaGames.length >= 2) {
      ...
      const proxyCount = parseProxyUrls().length
      const stepDefault = Math.max(50, Math.min(1000, Math.max(200, proxyCount * 10)))
      const stepCount = Math.max(50, Math.min(1000, envNum("ENKA_RR_STEP", stepDefault)))
  ...
  const runRefresh = async () => {
    ...
    const proxyEnabled = envBool("PROXY_ENABLED", Boolean(cfg2?.proxy?.enabled ?? false))
    let preservedProxyUrlsEnvRefresh = String(process.env.PROXY_URLS || "").trim()
    const bootstrapSubHttpProxyEnabled = envBool("PROXY_BOOTSTRAP_SUB_HTTP_PROXY", true)
    const bootstrapProxyUrl = String(process.env.PROXY_BOOTSTRAP_PROXY || cfg2?.proxy?.subscription?.httpProxy || "").trim()
    if (proxyEnabled && !preservedProxyUrlsEnvRefresh && bootstrapSubHttpProxyEnabled && bootstrapProxyUrl) {
      preservedProxyUrlsEnvRefresh = bootstrapProxyUrl
      process.env.PROXY_URLS = bootstrapProxyUrl
      logProxy.info(`启动代理：${bootstrapProxyUrl}`)
    }
    ...
    const proxyRequiredRefresh = envBool("PROXY_REQUIRED", Boolean(cfg2?.proxy?.required ?? false))
    const poolPromise = ensureProxyPool(cfg2, {
      ...
      .catch((e) => {
        ...
        if (proxyRequiredRefresh) throw e
        return null
      })
    ...
    try {
      // When proxy is required, enforce it before Enka sampling.
      if (proxyRequiredRefresh) await poolPromise
      ...
      const proxyEnabled = envBool("PROXY_ENABLED", Boolean(cfg2?.proxy?.enabled ?? false))
      const waitProxyEnabled = envBool("PROXY_WAIT_READY", Boolean(cfg2?.proxy?.subscription?.waitReady ?? true))
      const waitMin = Math.max(0, Math.min(50, envNum("PROXY_WAIT_READY_MIN", Number(cfg2?.proxy?.subscription?.waitReadyMin ?? 1))))
      const waitMs = Math.max(0, Math.min(5 * 60_000, envNum("PROXY_WAIT_READY_MS", Number(cfg2?.proxy?.subscription?.waitReadyMs ?? 60_000))))
      if (proxyEnabled && waitProxyEnabled && !proxyRequiredRefresh && enkaGames.length > 0 && waitMin > 0 && parseProxyUrls().length < waitMin) {
        logProxy.info(`等待代理就绪：min=${waitMin} timeoutMs=${waitMs}`)
        const ok = await waitForProxyUrls({ min: waitMin, timeoutMs: waitMs })
        const got = parseProxyUrls().length
        if (!ok) logProxy.warn(`等待代理就绪超时：got=${got}/${waitMin}；将继续直连/可能超时`)
      ...
      try {
        ...
        await pool?.close?.()
        // Restore env to prevent stale local proxies being used between refresh cycles.
        setProxyUrlsEnvRefresh([])
      } catch {}
    }
  }
