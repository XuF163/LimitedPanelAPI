...
import { updateDailyGateFromPreset } from "./samples/daily-gate.js"
import { loadSubscriptionNodes, nodeKey as subscriptionNodeKey, parseSubscriptionTextAsync } from "./proxy/subscription.js"
import { createLogger } from "./utils/log.js"
import { getProxyPool, getRuntimeStatus } from "./runtime/state.js"
import { scannerManager } from "./runtime/scanner.js"
...
function normalizeSimpleConfigPayload(input: any, effectiveCfg: any = {}) {
...
const uiIndexPath = firstExistingFile([
  path.join(__dirname, "ui", "index.html"),
  path.join(__dirname, "webui", "index.html")
])
const uiIndexHtml = (() => {
  try {
    if (!uiIndexPath) return null
...
const server = http.createServer((req, res) => {
  ...
  // v2: runtime status (observable only; no manual concurrency knobs)
  ...
  // v2: scanner control (start/stop background Enka scanning)
  // POST /api/samples/start
  if (url.pathname === "/api/samples/start") {
    const auth = authUi(req, cfg)
    if (!auth.ok) return sendJson(res, auth.code, { error: auth.error })
    if (req.method !== "POST") return sendJson(res, 405, { error: "method_not_allowed" })
    ;(async () => {
      ...
      const game = payload?.game || "all"
      await scannerManager.start({ game })
      return sendJson(res, 200, { ok: true })
    })().catch((e) => {
      return sendJson(res, 500, { error: "samples_start_failed", message: e?.message || String(e) })
    })
    return
  }

  // POST /api/samples/stop
  if (url.pathname === "/api/samples/stop") {
    const auth = authUi(req, cfg)
    if (!auth.ok) return sendJson(res, auth.code, { error: auth.error })
    if (req.method !== "POST") return sendJson(res, 405, { error: "method_not_allowed" })
    ;(async () => {
      ...
      const payload = body ? JSON.parse(body) : {}
      const game = payload?.game || "all"
      await scannerManager.stop({ game })
      return sendJson(res, 200, { ok: true })
    })().catch((e) => {
      return sendJson(res, 500, { error: "samples_stop_failed", message: e?.message || String(e) })
    })
    return
  }

  // v2: proxy pool status / rebuild
  // GET /api/proxy/pool/status
  if (url.pathname === "/api/proxy/pool/status") {
    const auth = authUi(req, cfg)
    if (!auth.ok) return sendJson(res, auth.code, { error: auth.error })
    if (req.method !== "GET") return sendJson(res, 405, { error: "method_not_allowed" })
    const pool = getProxyPool()
    if (!pool) return sendJson(res, 200, { ok: true, enabled: false, reason: "not_started" })
    try {
      const st = typeof pool.status === "function" ? pool.status() : { enabled: true, usable: Array.isArray(pool.proxyUrls) ? pool.proxyUrls.length : 0 }
      return sendJson(res, 200, { ok: true, ...st })
    } catch (e) {
      return sendJson(res, 500, { error: "proxy_pool_status_failed", message: e?.message || String(e) })
    }
  }

  // POST /api/proxy/pool/rebuild
  if (url.pathname === "/api/proxy/pool/rebuild") {
    const auth = authUi(req, cfg)
    if (!auth.ok) return sendJson(res, auth.code, { error: auth.error })
    if (req.method !== "POST") return sendJson(res, 405, { error: "method_not_allowed" })
    ;(async () => {
      const pool = getProxyPool()
      if (!pool) return sendJson(res, 200, { ok: false, error: "not_started" })
      if (typeof pool.rebuild !== "function") return sendJson(res, 200, { ok: false, error: "unsupported" })
      const ret = await pool.rebuild()
      return sendJson(res, 200, { ok: true, result: ret })
    })().catch((e) => {
      return sendJson(res, 500, { error: "proxy_pool_rebuild_failed", message: e?.message || String(e) })
    })
    return
  }

  // v2: preset generation (manual trigger)
  // POST /api/presets/generate
  if (url.pathname === "/api/presets/generate") {
    const auth = authUi(req, cfg)
    if (!auth.ok) return sendJson(res, auth.code, { error: auth.error })
    if (req.method !== "POST") return sendJson(res, 405, { error: "method_not_allowed" })
    ;(async () => {
      const body = await readBody(req, { maxBytes: 64 * 1024 })
      const payload = body ? JSON.parse(body) : {}
      const q = String(payload?.game || "all").toLowerCase()
      ...
      const limitChars = Number(cfg?.preset?.limitChars ?? 0) || 0

      for (const g of games) {
        const uid = g === "zzz"
          ? String(process.env.PRESET_UID_ZZZ || cfg?.preset?.uidZzz || "10000000")
          : String(process.env.PRESET_UID || cfg?.preset?.uid || "100000000")
        const outPath = path.join(paths.outDir(g), `${uid}.json`)
        const argv = [ "--game", g, "--uid", uid, "--name", name, "--out", outPath ]
        if (limitChars > 0) argv.push("--limitChars", String(limitChars))
        await cmdPresetGenerate(argv)
        await updateDailyGateFromPreset(g, cfg, { presetUid: uid, presetPath: outPath, force: false }).catch(() => {})
      }

      return sendJson(res, 200, { ok: true, game: q })
    })().catch((e) => {
      return sendJson(res, 500, { error: "presets_generate_failed", message: e?.message || String(e) })
    })
    return
  }
  ...
  if (url.pathname === "/api/proxy/import") {
    ...
    ;(async () => {
      ...
      if (subUrls.length) {
       ...
       if (rawText) {
        nodes.push(...(await parseSubscriptionTextAsync(rawText)))
       }
...
export async function startServer({ port, host }: any = {}) {
