...
function normalizeWeapon(raw: any = {}) {
  return {
...
function normalizeArtisMap(raw: any = {}, game) {
  const out = {}
...
export async function collectPlayerDataSamples({
  ...
  for (const name of files) {
    ...
    const raw: any = await loadJsonSafe(filePath)
    const uid = toInt(raw?.uid, toInt(name.replace(/\.json$/i, ""), 0))
    const avatars: any = raw?.avatars || {}

    for (const ds of Object.values(avatars) as any) {
      if (!ds?.id) continue
      scannedAvatars++
