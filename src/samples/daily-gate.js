...
function getGateConfig(cfg = {}, game) {
...
function isGateDoneRow(row, { threshold } = {}) {
...
async function calcAvatarMark(game, avatar, { meta } = {}) {
  const g = normalizeGame(game)
...
export async function shouldSkipEnkaScanToday(game, cfg, { force = false } = {}) {
  const g = normalizeGame(game)
...
export async function updateDailyGateFromPreset(game, cfg, { presetUid, presetPath, force = false } = {}) {
  const g = normalizeGame(game)
  const { enabled, threshold } = getGateConfig(cfg, g)
