...
export async function loadSrMeta(metaRoot = paths.metaSr) {
  const artifactMeta: any = await import(pathToFileURL(path.join(metaRoot, "artifact", "meta.js")).href)
  ...
  const artifactMetaJson: any = JSON.parse(await fsp.readFile(artifactMetaJsonPath, "utf8"))
  const characterData: any = JSON.parse(await fsp.readFile(characterDataPath, "utf8"))
  const weaponById: any = fs.existsSync(weaponDataPath) ? JSON.parse(await fsp.readFile(weaponDataPath, "utf8")) : {}

  const star5 = artifactMetaJson?.starData?.["5"] || artifactMetaJson?.starData?.[5] || null
  const subIdByKey: any = {}
  const subKeyById: any = {}
  for (const [ id, ds ] of Object.entries(star5?.sub || {}) as any) {
    const key = ds?.key
    if (!key) continue
