...
export async function loadGsMeta(metaRoot = paths.metaGs) {
  const artifactExtra: any = await import(pathToFileURL(path.join(metaRoot, "artifact", "extra.js")).href)
  const artifactMark: any = await import(pathToFileURL(path.join(metaRoot, "artifact", "artis-mark.js")).href)
  const artifactAlias: any = await import(pathToFileURL(path.join(metaRoot, "artifact", "alias.js")).href)
  ...
  const artifactData: any = JSON.parse(await fsp.readFile(artifactDataPath, "utf8"))
  const characterData: any = JSON.parse(await fsp.readFile(characterDataPath, "utf8"))
  ...
  const weaponById: any = {}
  for (const type of defaultWeaponTypes) {
    const p = path.join(metaRoot, "weapon", type, "data.json")
    if (!fs.existsSync(p)) continue
    const data: any = JSON.parse(await fsp.readFile(p, "utf8"))
    for (const [ id, ds ] of Object.entries(data || {}) as any) {
      weaponById[Number(id)] = { ...(ds || {}), type }
    }
  }

  const maxAppendIdByKey: any = {}
  for (const [ id, ds ] of Object.entries(artifactExtra.attrIdMap || {}) as any) {
  ...
  return {
    ...
    artifact: {
      ...
      artifactPieceById,
      artifactPieceByName,
      maxAppendIdByKey: Object.fromEntries(Object.entries(maxAppendIdByKey as any).map(([k, v]: any) => [ k, v.id ]))
    },
    character: {
      byId: characterData,
