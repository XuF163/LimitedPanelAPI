...
export function resolveZzzPluginRoot(cfg: any = {}) {
...
export function resolveZzzLocalPluginRoot(cfg: any = {}) {
  const pluginDir =
    process.env.ZZZ_PLUGIN_DIR ||
    cfg?.zzz?.source?.pluginDir ||
    path.join(getYunzaiRoot(), "plugins", "ZZZ-Plugin")
...
export function resolveZzzMapDir(cfg: any = {}) {
  const { pluginRoot } = resolveZzzPluginRoot(cfg)
  return path.join(pluginRoot, "resources", "map")
}

export function resolveZzzFormatterPath(cfg: any = {}) {
  const { pluginRoot } = resolveZzzPluginRoot(cfg)
...
async function runGit(args, options: any = {}) {
...
export async function ensureZzzSource(cfg: any = {}, { force = false }: any = {}) {
