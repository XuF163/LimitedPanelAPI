...
async function tryCopyMiaoPlayerDataPreset(game, uid, outPath, { quiet = false } = {}) {
  if (!["gs", "sr"].includes(String(game))) return false
  const src = path.join(guessYunzaiRoot(), "data", "PlayerData", String(game), `${uid}.json`)
  if (!fs.existsSync(src)) return false

  // Guard: ensure it's valid JSON to avoid copying corrupted files.
  try {
    JSON.parse(await fsp.readFile(src, "utf8"))
  } catch {
    return false
  }
  ...
  if (!quiet) log.info(`已复用 miao-plugin 产物：${src} -> ${outPath}`)
  return true
}
...
function ensureZzzProperties(avatar) {
  if (Array.isArray(avatar?.properties) && avatar.properties.length) return

  // Minimal fallback to keep ZZZ-Plugin scoring stable (avoids base_properties=0 -> Infinity weights).
  // Prefer plausible non-zero base stats; detailed/accurate properties should come from Enka2Mys samples.
  avatar.properties = [
    { property_name: "生命值", property_id: 111, base: "20000", add: "0", final: "20000" },
    { property_name: "攻击力", property_id: 121, base: "1000", add: "0", final: "1000" },
    { property_name: "防御力", property_id: 131, base: "800", add: "0", final: "800" },
    { property_name: "冲击力", property_id: 122, base: "120", add: "0", final: "120" },
    { property_name: "暴击率", property_id: 201, base: "0", add: "0", final: "50%" },
    { property_name: "暴击伤害", property_id: 211, base: "0", add: "0", final: "100%" },
    { property_name: "异常掌控", property_id: 312, base: "100", add: "0", final: "100" },
    { property_name: "异常精通", property_id: 314, base: "100", add: "0", final: "100" },
    { property_name: "穿透率", property_id: 231, base: "0", add: "0", final: "20%" },
    { property_name: "能量自动回复", property_id: 305, base: "0", add: "0", final: "1.0" }
  ]
}

function ensureZzzSkills(avatar) {
  // ZZZ-Plugin expects `skills` as an array of { level, skill_type, items }.
  // Enka ZZZ skill indices are: 0,1,2,3,5,6 (note: 4 is unused).
  const rank = Number(avatar?.rank) || 0
  const rankLevel = rank >= 3 ? (rank >= 5 ? 4 : 2) : 0

  avatar.skills = [0, 1, 2, 3, 5, 6].map((skill_type) => ({
    level: 12 + (rankLevel && skill_type !== 5 ? rankLevel : 0),
    skill_type,
    items: []
  }))
}
...
export async function cmdPresetGenerate(argv) {
  ...
  // GS/SR: prefer miao-plugin extreme panel output (PlayerData UID preset) when present,
  // so results stay consistent with miao-plugin's generation.
  if (args.game === "gs" || args.game === "sr") {
    const outPath = args.out || path.join(paths.outDir(args.game), `${args.uid}.json`)
    if (await tryCopyMiaoPlayerDataPreset(args.game, args.uid, outPath, { quiet: args.quiet })) return
  }
  ...
  if (args.game === "gs") {
    ...
    if (!fs.existsSync(sampleDir)) {
      throw new Error(`缺少样本目录：${sampleDir}（请先运行：node dist/cli.js sample:collect ...）`)
    }
    const charIds = fs
    ...
    if (ids.length === 0) {
      throw new Error(`未找到任何样本文件：${sampleDir}（请先运行：node dist/cli.js sample:collect ...）`)
    }
  ...
  if (args.game === "zzz") {
    ...
    for (const id of ids) {
      ...
      ensureZzzProperties(outAvatar)
      ensureZzzSkills(outAvatar)

      result.avatars[String(charId)] = outAvatar

      const extremeMark = calcZzzAvatarMark(outAvatar)
