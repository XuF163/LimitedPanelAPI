...
export function openScanDb({ dbPath = "" } = {}) {
  ...
  const markPermanent = (uid, { game = "gs", status = null, error = null } = {}) => {
    ...
    const row = {
      ...
      fail_count: Number(prev?.fail_count || 0) + 1,
  ...
  const recordSuccess = (uid, { game = "gs", status = 200, bodyText = null, storeRaw = true } = {}) => {
    const g = normalizeGame(game)
    const prev = getUidState(uid, { game: g })
    stmtUpsert.run({
      game: g,
  ...
  const recordFailure = (uid, { game = "gs", status = null, error = null, retryAfterMs = null } = {}) => {
    const g = normalizeGame(game)
    const prev = getUidState(uid, { game: g })
    const failCount = Number(prev?.fail_count || 0) + 1
  ...
  const setDailyGate = (
    game,
    day,
    { done = false, doneAt = null, totalChars = null, qualifiedChars = null, detailJson = null } = {}
  ) => {
    const g = normalizeGame(game)
    const d = String(day || "").trim()
    if (!g || !d) return
