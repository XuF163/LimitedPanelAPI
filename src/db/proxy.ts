...
export function openProxyDb({ dbPath = "" } = {}) {
  ...
  // Avoid "database is locked" when multiple processes share the same proxy DB
  // (e.g. start.js + WebUI refresh, or multiple bots sharing temp/LimitedPanelAPI).
  ...
  const insertNode = ({ key = "", node = null } = {}) => {
  ...
  const finishAttempt = (id, { ok = null, status = null, ms = null, error = null } = {}) => {
    if (!id) return
    stmtUpdate.run(
