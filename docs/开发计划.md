...
- 除 `README.md` 外，所有文档统一放到 `docs/`：
  ...
  - `docs/重构计划.md`：重构路线（TS v2 / 弃用 Rust / 智能并发与代理池）
  - `docs/v2-实施清单.md`：实施步骤与验收口径（直到“重构完成”）
...
2. `EnkaClient`
   ...
   - 直连 +（可选）代理池：
     ...
     - 有代理时：按代理数量分桶节流 + 自适应并发采样（失败触发指数退避）
3. `SampleCollector`
...
- 可选：UID 枚举（例如从 `242422996` 开始）
  - 必须具备强约束：最大请求数、速率预算（节流/退避）、失败率熔断（并发不允许手动上限）
...
- `POST /samples/collect`
  - body：`{ game, uids?: number[], uidStart?: number, count?: number }`
...
1. **Enka 合规与限速**：不要默认扫段；扫段必须显式开启并设置上限（总量/熔断），并通过“节流 + 指数退避”保证请求速率不会失控。
...
1. 初始化项目骨架（TypeScript）+ `GET /healthz` + 静态托管 `out/`
