...
> 提示：下文接口分为“已存在（当前实现）”与“已实现（v2）”。  
> v2 的重点是：**自适应并发 + 指数退避**（不允许手动设置并发上限）与 **动态代理池** 的可观测/可控。
...
> 说明：本节为 v2 规划接口，用于 WebUI 展示状态；不提供“手动并发上限”调参入口。

### GET `/api/runtime/status`

用途：WebUI 总览页显示运行状态（ProxyPool / Scanner / Preset / 自适应并发与退避）。
...
```json
{
  ...
  "scanner": {
    "running": true,
    "games": {
      "gs": { "enabled": true, "cursor": 100000001, "doneToday": false },
      "sr": { "enabled": false },
      "zzz": { "enabled": false }
    },
    "recent1m": { "ok": 80, "http429": 2, "timeout": 5, "htmlWaf": 1 }
  }
}
```
...
## 已实现（v2）：采样任务控制（不含并发上限）

### POST `/api/samples/start`

用途：启动（或恢复）扫描任务。  
说明：并发由系统自适应调节；失败将触发指数退避。
...
用途：手动触发生成极限面板（按当前样本与配置写入 `out/<game>/<uid>.json`）。
...
用途：WebUI 展示代理池状态（目标数/可用数/失败原因分布/禁用统计）。

### POST `/api/proxy/pool/rebuild`

用途：强制重建池（重新探测候选并补齐）。  
说明：重建池不等同于提升并发上限；并发仍由系统自适应调节。
...
- v2 已实现采样并发为“自适应 + 指数退避”，不再允许通过配置设置并发上限，因此 `samples.enka.concurrency/fetcher` 等字段将被移除/忽略
